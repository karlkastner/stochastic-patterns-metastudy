...
function [stat,file_C,Si,Ri] = pattern_metastudy_analyze(id,meta)
    ...
    for idx=id
        ...
        if (~isempty(file_C{idx}))
            ...
            % if the pattern was generated with a model having scale-dependent feedback
            ...
            % if the pattern is isotropic (1) or anisotropic (0)
            % this field is for quality control of the automatic classification
            if (regexp(file_C{idx},'iso_[0-9]'))
                isisoman_str=regexprep(file_C{idx},'.*iso_([0-9]*).*','$1');
                isisoman = str2double(isisoman_str);
            else
                isisoman = 0;
            end
            ...
            if (meta.dflag)
                ...
                if (sp.stat.isisotropic)
                    ...
                    ylim([0,1.05*sp.stat.Sc.xp.hp*sp.stat.fc.x.hp]);
                    end
    ...
    if (meta.saveflag)
        ...
        name = [meta.filename.metastudy(1:end-4),'-',datestr(now(),'yyyy-mm-dd'),'.mat'];
        copyfile(meta.filename.metastudy,name);
    end
